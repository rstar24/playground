...
public:
  ...
  friend StrType operator+(char *s,
                           StrType &o); // Concatenate the a quoted string
...
int main(int argc, char *argv[]) {
  ...
  if (s1 < s4)
    cout << "s1 less than s4" << endl;
  if (s1 > s4)
    cout << "s1 is greater than s4" << endl;
  if (s1 <= s4)
    cout << "s1 less than or equal to s4" << endl;
  if (s1 >= s4)
    cout << "s1 is greater than or equal to s4" << endl;

  if (s2 > "ABC")
    cout << "s2 is greater than ABC" << endl;
