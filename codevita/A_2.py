...
#--FUNCTIONS BEGINS--
def solve(inst:list) -> int:
    inst_1=[]
    for i in range(len(inst)):
        if (inst[i] == 'down' or inst[i] == 'up'):
            inst_1.append(inst[i])

    i = 0
    count = 0
    n = len(inst_1)
    while (i < n-1):
        if ((inst_1[i] == 'down' and inst_1[i+1] == 'up') or 
              (inst_1[i] == 'up' and inst_1[i+1] == 'down')):
            i = i + 1
            count = count + 1
        else:
            i = i + 1
...
def main():
    ...
    for i in range(tcs):
        temp = str(input())
        instrutction.append(temp)
    print(solve(instrutction))
